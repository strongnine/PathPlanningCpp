...
void BestFirst::init(Coordinate start, Coordinate goal, int type) {
    start_ = start;
    goal_ = goal;
    heuristic_type_ = type;
}

void BestFirst::searching(std::vector<Coordinate>& path,
    ...
    pq.push(CoordinatePair(heuristic(start_), start_));
        ...
        for (auto dir : dirs) {
            ...
            if (new_x >= 0 && new_x < x_range_ && new_y >= 0 &&
                ...
                if (new_cost < cost) {
                    ...
                    float priority = heuristic(new_point);
...
float BestFirst::heuristic(const Coordinate& p) {
    ...
    if (heuristic_type_ == MANHATTAN) {
        ...
        res = std::hypot(goal_.x - p.x, goal_.y - p.y);
    }
    return res;
}
